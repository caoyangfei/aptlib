...
import com.flyang.plugin.aspectj.internal.model.JarInfo
...
class VariantCache {
    ...
    VariantCache(Project proj, AspectjCache cache, String variantName) {
        ...
        this.aspectJCache = cache
        this.aspectJCache.put(variantName, this)
    ...
    private void init() {
        cachePath = project.buildDir.absolutePath + File.separator + AndroidProject.FD_INTERMEDIATES + "/aspectJ/" + variantName
        ...
        if (includeJarConfig.exists()) {
            List<JarInfo> jarInfoList = AspectjUtils.optFromJsonString(FileUtils.readFileToString(includeJarConfig), new TypeToken<List<JarInfo>>(){}.getType())
            if (jarInfoList != null) {
    ...
    void commitIncludeJarConfig() {
        ...
        FileUtils.write(includeJarConfig, AspectjUtils.optToJsonString(jarInfoList), "UTF-8")
    }
