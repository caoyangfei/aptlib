...
class CheckAspectJXEnableProcedure extends AbsProcedure {
    ...
    boolean doWorkContinuously() {
        ...
        for (String filter : aspectJExtensionConfig.excludes) {
            if (filter == "*" || filter == "**") {
                isExcludeAll = true
                break
            }
        ...
        for (String filter : aspectJExtensionConfig.includes) {
            if (filter == "*" || filter == "**") {
                isIncludeAll = true
                break
            }
        ...
        if (isIncludeAll) {
            aspectJExtensionConfig.includes.clear()
        }

        //aspectjx disabled
        if (!aspectJExtensionConfig.enabled || isExcludeAll) {
